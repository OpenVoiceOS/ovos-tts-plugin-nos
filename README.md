...
For the `sabela`, `icia`, `iago`, `paulo` cotovia-phonemized voices, `cotovia` must be available in your base OS — same requirement as before. Follow the install steps in [ovos-tts-plugin-cotovia](https://github.com/OpenVoiceOS/ovos-tts-plugin-cotovia).
...
| — | `proxectonos/brais` | graphemes (new, no cotovia) |
...
All voices are `gl-ES` (Galician). The cotovia-free `proxectonos/celtia` and `proxectonos/brais` voices are recommended for systems without cotovia installed.
